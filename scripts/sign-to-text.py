...
OLLAMA_MODEL      = "llama3.2"  # or 'llama3' or 'mistral'
...
PAUSE_SECONDS       = 5.0   # seconds of inactivity before triggering translation
...
def run():
    ...
    try:
        with open(LABELS_PATH, 'r') as f:
            raw = json.load(f)
        # If {"sign": index} → flip to {index: "sign"}
    ...
    hand_absent_since    = None   # timestamp when hands first disappeared
    global_action        = None
    ...
    frame_h, frame_w = 480, 640  # default, updated each loop

    print("\n" + "="*50)
    print("SIGN LANGUAGE RECOGNITION — RUNNING")
    ...
    with mp_holistic.Holistic(
        ...
        while cap.isOpened():
            ...
            if hands_present:
                ...
                hand_gap_counter  = 0
                hand_absent_since = None   # reset absence timer the moment hand is seen
            else:
                hand_gap_counter += 1
                if hand_gap_counter > MAX_HAND_GAP_FRAMES:
                    # Truly gone — reset inference state but keep absence timer
                    consecutive_hand_frm = 0
                    sequence.clear()
                    vote_buffer.clear()
                    stability_counter = 0
            ...
            if (global_cooldown_remaining <= 0
                ...
                if len(seq_list) < SEQUENCE_LENGTH:
                    # Replicate the last frame to fill the window
                ...
                if top_prob > CONFIDENCE_THRESH:
                    ...
                    if len(vote_buffer) >= VOTE_BUFFER_SIZE:
                        ...
                        if count >= STABILITY_FRAMES:
                            ...
                            # Accept the sign if cooldown has expired
                            if winner != last_predicted_word and winner not in sign_cooldown:
                                ...
                                vote_buffer.clear()   # reset so next distinct sign can accumulate
            ...
            # ── Pause → trigger LLM ───────────────────────────────────────────
            # Only fire when hands have been CONTINUOUSLY absent for PAUSE_SECONDS
            ...
            if word_buffer and hands_gone_long_enough:
                ...
                current_candidate   = None
                stability_counter   = 0
                hand_absent_since   = None  # reset so timer doesn't re-fire immediately
            ...
            # Candidate progress bar (stability)
